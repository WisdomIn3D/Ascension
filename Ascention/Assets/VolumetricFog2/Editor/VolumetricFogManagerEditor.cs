    ...
    public class VolumetricFogManagerEditor : Editor {
        ...
        public override void OnInspectorGUI() {
            ...
            if (blurPasses.intValue > 0) {
                ...
                EditorGUILayout.HelpBox("Only use this blur option if you need it. When blur enabled, fog volumes ignore render queue value. Select the render pass event in the Volumetric Fog Render Feature.", MessageType.Info);
            } else if (VolumetricFogRenderFeature.installed) {
                EditorGUILayout.HelpBox("No blur used. Remove 'Volumetric Fog Render Feature' from the Forward Renderer of the Universal Rendering Pipeline asset to save performance.", MessageType.Warning);
                if (pipe != null && GUILayout.Button("Show Pipeline Asset")) Selection.activeObject = pipe;
            }
